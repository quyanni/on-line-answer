...
public class MessageDao {
	public boolean saveMsg(String title, String content, int cid, String username,int statue) throws SQLException {
	...
	public List<Message> showDifficulties(String username) throws SQLException {
		Connection conn = JDBCUtil.getConnection();
		String sql = "select * from message, course, teacher where message.cid=course.cid and course.tuname=teacher.username "
				+ "and status=1 and teacher.username=? order by date desc";
		PreparedStatement ps = conn.prepareStatement(sql);
		ps.setString(1, username);
		ResultSet rs = ps.executeQuery();
		List<Message> list = new ArrayList<>();
